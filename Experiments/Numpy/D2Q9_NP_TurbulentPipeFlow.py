...
timeSteps = 200
stepsPerFrame = 100
Re = 5000.0 ### Reynolds number (adjust for viscosity)

nx = 420 ### Lattice Dimensions
ny = 180

uy,dy = ny//4, 3*ny//4 ### Constants for obstacle (Example for flow around a cylinder)
...
def inObstacle(x,y) : ### Boolean function for obstacle (Example for flow around a cylinder)
    return [[((y[a,b] < uy) and (y[a,b] < x[a,b])) or ((y[a,b] >= dy) and (ny-y[a,b] < x[a,b])) for b in range(ny)] for a in range(nx)]
